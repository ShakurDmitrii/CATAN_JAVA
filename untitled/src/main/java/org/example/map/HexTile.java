...
public class HexTile {
    ...
    private float[][] vertices = new float[6][2]; // координаты вершины
    ...
    public HexTile(float x, float y, String type, int number, Player owner) {
        ...
        computeVertices(50); // ← вычисляем вершины гекса
    }
    // HexTile.java
    public int getNearestVertex(float mx, float my) {
        for (int i = 0; i < 6; i++) {
            float vx = vertices[i][0];
            float vy = vertices[i][1];

            float dist = (float) Math.sqrt((mx - vx) * (mx - vx) + (my - vy) * (my - vy));

            if (dist < 15) { // радиус попадания
                return i;

            }break;
        }
        return -1; // если нет попадания
    }
